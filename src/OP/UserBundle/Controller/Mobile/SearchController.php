...
class SearchController extends Controller
{
    ...
    public function loadUsersAction($name)
    {
        $dbUsers = $this->get('op_user.user_manager')->loadUsers($name);
        $datas = [];
    ...
    public function loadOnlineUsersAction()
    {
        $dbUsers = $this->get('op_user.user_manager')->lessUserSuggest($this->_getUser());
        $datas = [];
    ...
    public function lessUserSuggestAction(Request $request)
    {
        $helper = new FlushHelper();
        
        return new StreamedResponse (function() use ($helper, $request){
            $auth_user = $this->_getUser();
            $user_suggests = $this->get('op_user.user_manager')->lessUserSuggest($auth_user);
            $dm     = $this->dm;
    ...
    public function getFriendsOrFollowersAction(){
        
        $dbUsers = $this->get('op_user.user_manager')->friendsOrFollowers();
        $datas = [];
        foreach ($dbUsers as $dbUser) {
            $user['userId'] = $dbUser->getId();
            $user['username'] = $dbUser->getUsername();
