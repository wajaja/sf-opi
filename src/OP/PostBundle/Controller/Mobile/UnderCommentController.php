...
class UnderCommentController extends Controller
{
     ...
     * @return type
     */
    public function listUnderCommentsAction(Request $request, $commentId, ToArrayTransformer $transformer){
        $dm         = $this->getDocumentManager();
        $underCrepo = $dm->getRepository('OPPostBundle:UnderComment');
        if($request->isXmlHttpRequest()){
            $res = new JsonResponse();
            $listComment = $underCrepo->xhrUnderComments($commentId);
            $listComments =  $transformer->underCommentToArray($listComment);
        ...
        return $this->render(
            'OPPostBundle:UnderComment:listComments.html.twig', 
            array('listComments'=>$listComments, 
                  'user'=>  $this->_getUser()
            )
        );
    }
