...
class PostListener
{
	...
	private $post, $comment, $undercomment, $container,
			$request, $transformer, $stream;
	
	public function __construct(RequestStack $request, Container $container, ToArrayTransformer $transformer, Stream $stream)
	{
  ...
  //       //$session->set('gallerycomment', []); 	//reset uploaded filename in session;
	...
	public function onLike(Events\LikeEvent $event)
	{
		...
		try {
			$pusher     = $this->container->get('gos_web_socket.wamp.pusher');
			$pusher->push(
				['data' => $data], 'like_topic', ['post_id' => $data['id'], 
												  'refer'   => $data['refer']]
			);
		} catch (Exception $e) { }
        //Redis to send post to all followers
	...
	public function onRateCreate(Events\RateEvent $event)
	{
		...
		try {
			$pusher     = $this->container->get('gos_web_socket.wamp.pusher');
			$pusher->push(
				['data' => $data], 'rate_topic', ['post_id' => $data['id'], 
												  'refer'   => $data['refer']]
			);
		} catch (Exception $e) { }
        //Redis to send post to all followers
