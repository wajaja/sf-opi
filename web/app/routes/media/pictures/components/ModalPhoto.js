...
const ModalPhoto  = createReactClass({
    ...
    onShare(e) {
        e.preventDefault();
        this.props.onShare(this.state.photo.id, 'photo');
    },
    ...
    goPrev () {
        ...
        this.props.history.push(`/pictures/${prevId}?post_id=${photo.postValid}`,
                                    {modal: true, returnTo: (returnTo -1) }
                                   )
    },
    ...
    goNext () {
        ...
        returnTo = this.props.location.state.returnTo;
        this.props.history.push(`/pictures/${nextId}?post_id=${photo.postValid}`, 
                                    {modal: true, returnTo: (returnTo -1) }
                                  )
    },
    ...
    closeModal(e) {
        ...
        returnTo = location.state ? location.state.returnTo : undefined;
        if(returnTo === undefined) 
            console.log('no back');

        this.props.dispatch(PhotoActions.closeModal())
        this.props.history.go(returnTo);
    },
     ...
     */
    render() {
        const { selected_activity, showSecrets, questionId,
                                ...
                                <div className="ph-ctnr-b">
                                    ...
                                    <div className="mdl-side ph-rght">
                                        ...
                                        <Right 
                                            ...
                                            renderLikes={this.renderLikes}
                                            showSecrets={showSecrets}
                                            showComments={showComments}
                                            loadingSecret={loadingSecret}
                                            renderSecrets={this.renderSecrets}
                                            renderComments={this.renderComments}
                                            changeQuestionId={this.changeQuestionId}
                                            selected_activity={selected_activity}
                                            secretAlreadySeen={secretAlreadySeen}
                                            />
                                    </div>
